...
const ButtonMain: React.FC<ButtonReserveProps> = ({ label, link, className, type = "button" }) => {
  ...
  return link ? (
    <Link href={link}>
      <button className={`${className} float-right w-40 group overflow-hidden btn_base py-2 px-3 rounded items-center flex gap-2 bg-gradient-to-r from-[#B38425] to-[#D6B456] text-light-gray  hover:bg-primary-black hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0`}>
        {buttonContent}
      </button>
    </Link>
  ) : (
    <button type={type} className={`${className} float-right w-40 group overflow-hidden btn_base py-2 px-3 rounded items-center flex gap-2 bg-gradient-to-r from-[#B38425] to-[#D6B456] text-light-gray hover:text-primary-black hover:bg-primary-black hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0`}>
      {buttonContent}
    </button>
  );
};
...
/* export default function ButtonMain({ label, link, className, type  }:any) {
  return (
    <Link href={link} className={`${className} float-right w-40 group overflow-hidden btn_base py-2 px-3 rounded items-center flex gap-2 bg-gradient-to-r from-[#B38425] to-[#D6B456] text-light-gray hover:text-primary-black hover:bg-primary-black hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 active:translate-y-0 `}>
